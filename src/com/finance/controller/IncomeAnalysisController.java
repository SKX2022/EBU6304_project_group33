...
public class IncomeAnalysisController implements Initializable {
    ...
    @FXML private PieChart taxBredownChart;
    ...
    private void initializeChart() {
        taxBredownChart.setTitle("税收构成分析");
        taxBredownChart.setLegendVisible(true);
        taxBredownChart.setLabelsVisible(true);
    }
     ...
     * 更新饼图
     */
    private void updateChart(TaxCalculationResult result) {
        ObservableList<PieChart.Data> pieChartData = FXCollections.observableArrayList();
        
        double netIncome = result.getNetAnnualIncome();
        double incomeTax = result.getIncomeTax();
        double socialSecurity = result.getSocialSecurity();
        double housingFund = result.getHousingFund();
        
        // 只显示非零的项目
        if (netIncome > 0) {
            pieChartData.add(new PieChart.Data("实际到手 (¥" + currencyFormat.format(netIncome) + ")", netIncome));
        }
        
        if (incomeTax > 0) {
            pieChartData.add(new PieChart.Data("个人所得税 (¥" + currencyFormat.format(incomeTax) + ")", incomeTax));
        }
        
        if (socialSecurity > 0) {
            pieChartData.add(new PieChart.Data("社保费用 (¥" + currencyFormat.format(socialSecurity) + ")", socialSecurity));
        }
        
        if (housingFund > 0) {
            pieChartData.add(new PieChart.Data("公积金 (¥" + currencyFormat.format(housingFund) + ")", housingFund));
        }
        
        taxBredownChart.setData(pieChartData);
        
        // 设置图表样式
        taxBredownChart.setClockwise(true);
        taxBredownChart.setLabelLineLength(10);
        taxBredownChart.setStartAngle(90);
    }
    ...
    private void resetForm(ActionEvent event) {
        ...
        // 清空图表
        taxBredownChart.setData(FXCollections.observableArrayList());
